...
// Função para salvar os dados de mobilização no localStorage
function saveMobilization() {
    ...
    // Calcular a duração da mobilização
    ...
    // Criar objeto com os dados da mobilização
    ...
    let mobilizations = JSON.parse(localStorage.getItem('mobilizations')) || [];

    // Adicionar nova mobilização
    mobilizations.push(mobilizationData);

    // Salvar no localStorage
    localStorage.setItem('mobilizations', JSON.stringify(mobilizations));

    // Informar ao usuário que os dados foram salvos
    alert('Mobilização registrada com sucesso!');
...
function clearForm() {
    ...
    document.getElementById('activity').value = 'Montagem do equipamento';
